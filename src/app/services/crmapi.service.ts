...
export class CrmapiService {
  ...
  addCrmApis(item): Observable<CrmapiListsItem> {
    return this.http.post<CrmapiListsItem>(Config.URLS.crmapiAdd, item).pipe(
      tap((crmadd: CrmapiListsItem) =>
        console.log(`added crm api  id=${crmadd.id}`)
      ),
  ...
  activateCrmapi(apiId, status): Observable<CrmapiListsItem> {
    return this.http
      .post<CrmapiListsItem>(Config.URLS.crmapiActivate, {
        id: apiId,
        activate: status
      })
      .pipe(
        tap((crmadd: CrmapiListsItem) =>
          console.log(`activated crm api  id=${apiId}`)
        ),
        catchError((error: HttpError) => {
          this.notification.error(error.message);
  ...
  deleteCrmApi(apiId): Observable<CrmapiListsItem> {
    return this.http
      .post<CrmapiListsItem>(Config.URLS.crmapiDelete, { id: apiId })
      .pipe(
        tap((crmadd: CrmapiListsItem) =>
          console.log(`deleted crm api  id=${apiId}`)
        ),
        catchError((error: HttpError) => {
          this.notification.error(error.message);
  ...
  public getCrmapi(): Observable<{ data: CrmapiListsItem[] }> {
    return this.http.get<{ data: CrmapiListsItem[] }>(
      Config.URLS.apiListCrmApis
    );
  }
